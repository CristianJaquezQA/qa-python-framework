...
BASE = "https://jsonplaceholder.typicode.com"  # replace with your API base URL
TIMEOUT = 10
...
def test_create_post_returns_201_and_body():
    payload = {"title": "demo", "body": "content", "userId": 1}
    r = requests.post(f"{BASE}/posts", json=payload, timeout=TIMEOUT)
    assert r.status_code in (200, 201)
    body = r.json()
    assert body["title"] == payload["title"]
    assert "id" in body  # backend assigns an id


def test_update_post_put_idempotent():
    payload = {"id": 1, "title": "new", "body": "text", "userId": 1}
